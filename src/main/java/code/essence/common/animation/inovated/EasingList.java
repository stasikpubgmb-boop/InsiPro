package code.essence.common.animation.inovated;
...
public class EasingList {
    ...
    public static Easing powIN_OUT(double n) {
        return (value) -> (float) (value < 0.5D ? Math.pow(2.0D, n - 1.0D) * Math.pow(value, n) : 1.0D - Math.pow(-2.0D * value + 2.0D, n) / 2.0D);
    }
