package code.essence.utils.display.color;
...
import static code.essence.utils.math.calc.Calculate.clamp;
...
public class ColorAssist {
    ...
    public static int clampSaturation(int color, float min, float max) {
        float[] hsb = Color.RGBtoHSB(getRed(color), getGreen(color), getBlue(color), new float[3]);
        return Color.HSBtoRGB(hsb[0], Math.clamp(hsb[1], min, max), hsb[2]);
    }
    ...
    public static int multSaturation(int color, float saturation) {
        float[] hsb = Color.RGBtoHSB(getRed(color), getGreen(color), getBlue(color), new float[3]);
        return Color.HSBtoRGB(hsb[0], clamp(hsb[1] + saturation, 0f, 1f), hsb[2]);
    }
    ...
    public int getRainbowColor(int color1, int color2) {
        long time = System.currentTimeMillis();
        float t = (float) (Math.sin(time / 500.0 * 0.001) + 1.0) / 2.0f;
        return interpolateColor(color1, color2, t);
    }

    public int getDynamicRainbowColor(int color1, int color2) {
        long time = System.currentTimeMillis();
        float speed = 500.0f * 0.001f;
        
        float t = (float) (Math.sin(time / speed) + 1.0) / 2.0f;
    ...
    public int getTrueRainbowColor() {
        long time = System.currentTimeMillis();
        float speed = 500.0f * 0.001f;
        
        float hue = (time / speed) % 360.0f;
        return Color.HSBtoRGB(hue / 360.0f, 0.8f, 1.0f);
    }

    public int getGradientRainbowColor(int color1, int color2) {
        long time = System.currentTimeMillis();
        float speed = 500.0f * 0.001f;
        
        float t = (float) (Math.sin(time / speed) + 1.0) / 2.0f;
        
        float rainbowMod = (float) (Math.sin(time / speed * 2.0f) * 0.1f + 0.9f);
    ...
    public int getSmoothRainbowColor(int color1, int color2) {
        long time = System.currentTimeMillis();
        float speed = 500.0f * 0.001f;
    ...
    public int getSimpleRainbowColor(int color1, int color2) {
        long time = System.currentTimeMillis();
        float speed = 500.0f;
        
        float t = (float) (Math.sin(time / (speed * 0.1f)) + 1.0) / 2.0f;
        
        
        return interpolateColor(color1, color2, t);
    }

    public int getTrueRainbowColor(int color1, int color2) {
        return fade(5, 0, color1, color2);
    }

    public int getFastRainbowColor(int color1, int color2) {
        long time = System.currentTimeMillis();
        
        float hue = (time / 10.0f) % 360.0f;
        int color = Color.HSBtoRGB(hue / 360.0f, 0.8f, 1.0f);
        
        
        return color;
    }

    public int getDebugRainbowColor(int color1, int color2) {
        long time = System.currentTimeMillis();
        float speed = 500.0f * 0.001f;
        
        float t = (float) (Math.sin(time / (speed * 100.0f)) + 1.0) / 2.0f;
        
        if (time % 1000 < 50) {
            System.out.println("Rainbow t: " + t + ", speed: " + speed + ", time: " + time);
        }
        
        return interpolateColor(color1, color2, t);
    }

    public int interpolateColor(int colorStart, int colorEnd, float t) {
    ...
    public int getClientColor() {
        return ThemeManager.primaryColor.getColor();
    }

    public int getClientColor2() {
        return ThemeManager.secondaryColor.getColor();
    }

    public int getClientColor2(float alpha) {
        return multAlpha(ThemeManager.secondaryColor.getColor(), alpha);
    }
