...
public class ConfigInputComponent extends AbstractComponent implements InputManager.InputComponent, InputManager.ClearableInput {
    ...
    private final ConfigDisplayComponent configDisplay;

    public ConfigInputComponent() {
        this.configDisplay = new ConfigDisplayComponent();
    }
    ...
    public void render(DrawContext context, int mouseX, int mouseY, float delta) {
        ...
        String buttonAnimationKey = "config_add_button_" + System.identityHashCode(this);
        float buttonScale = AnimationHelper.getAnimationValue(buttonAnimationKey, 1f);
        
        rectangle.render(ShapeProperties.create(matrix, squareX + 12, squareY, squareSize , squareSize)
                .round(4f)
        ...
        Fonts.getSize(18, Fonts.Type.ESSENCE).drawString(context.getMatrices(),
                "y", iconX+12.3, iconY + 7.6, ThemeManager.textColor.getColor());

        String displayText = text.equalsIgnoreCase("") && !typing ? "New Config..." : text;
        ...
        configDisplay.x = x;
        configDisplay.y = y - 11;
        configDisplay.width = width;
        configDisplay.height = 20;
        configDisplay.render(context, mouseX, mouseY, delta);
    }
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int button) {
        ...
        configDisplay.width = width;
        configDisplay.height = 20;
        if (configDisplay.mouseClicked(mouseX, mouseY, button)) {
            return true;
        }
    ...
    private void saveConfig() {
        if (text.isEmpty()) {
            System.out.println("ConfigInputComponent: Cannot save config - name is empty");
            return;
        }

        try {
            String addAnimationKey = "config_add_" + text;
            AnimationHelper.startAnimation(addAnimationKey, 0f, 1f, 300, AnimationHelper.EasingType.BOUNCE_OUT);
            AnimationHelper.startAnimation(addAnimationKey + "_alpha", 0f, 1f, 300, AnimationHelper.EasingType.BOUNCE_OUT);
            ...
            System.out.println("ConfigInputComponent: Error saving config: " + e.getMessage());
            e.printStackTrace();
        }
    }
