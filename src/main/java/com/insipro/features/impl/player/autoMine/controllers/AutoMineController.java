...
import com.insipro.features.impl.player.autoMine.api.MineApiParser;
import com.insipro.features.impl.player.autoMine.state.AutoMineState;
...
public class AutoMineController implements QuickImports {
    ...
    public void onRotationUpdate(RotationUpdateEvent event) {
        switch (state) {
            case RUNNING_TO_CENTER -> {
                Vec3d center = regionController.getRegionState().getCenterVec();
                com.insipro.utils.features.aura.warp.Turns turns =
                        com.insipro.utils.features.aura.utils.MathAngle.calculateAngle(center);
                TurnsConnection.INSTANCE.rotateTo(turns,
                        new com.insipro.utils.features.aura.warp.TurnsConfig(
                                mineController.getSpAngle(), true, false),
                        com.insipro.utils.math.task.TaskPriority.HIGH_IMPORTANCE_1,
                        com.insipro.Essence.getInstance().getModuleProvider()
                                .get(com.insipro.features.impl.player.autoMine.receivers.AutoMineModule.class));
            }
